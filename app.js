...
async function importarJogos() {
  ...
  ).map(j => ({ id: "j" + Date.now() + Math.random().toString(36).slice(2), data: j.data, time_a: j.timeA, time_b: j.timeB, rodada: j.rodada }));

  if (!novos.length) return toast("Todos os jogos já estão cadastrados!", true);
  try {
    // insert in batches of 20
    for (let i = 0; i < novos.length; i += 20) {
      await db.from("jogos").insert(novos.slice(i, i+20));
    }
    await recarregarTabela("jogos");
    renderAdminJogos();
    populateAdminSelects();
