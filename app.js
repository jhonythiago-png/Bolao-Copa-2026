      ...
      return `
        <div class="ranking-card ${top3cls}" onclick="openPlayerModal('${p.id}')">
          <div class="ranking-card-left">
            ...
            <div class="ranking-card-avatar" style="background:${color}">${getAvatar(p.nome)}</div>
          ...
          <div class="ranking-card-pts">${p.totalPts}<span class="ranking-card-pts-lbl">pts</span></div>
        </div>`;
...
function renderPremiacao() {
  ...
  if (grid) {
    grid.innerHTML = PREMIOS_PCT.map((p, i) => {
      ...
      const part    = ranked[i];
      const cor     = CORES[i];
      const avatar  = part ? `<div class="premio-avatar" style="background:${getAvatarColor(part.nome)}">${getAvatar(part.nome)}</div>` : `<div class="premio-avatar" style="background:var(--bg3);color:var(--muted)">?</div>`;
      return `
        <div class="premio-card" style="--cor:${cor}">
          <div class="premio-card-top">
            <span class="premio-emoji">${EMOJIS[i]}</span>
