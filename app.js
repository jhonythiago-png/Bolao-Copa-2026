...
const LiveSync = (() => {
  ...
  function atualizarBadge(aoVivo) {
    ...
    const hora = _lastSync ? _lastSync.toLocaleTimeString("pt-BR", { hour:"2-digit", minute:"2-digit", second:"2-digit" }) : "--:--:--";
    if (aoVivo) {
  ...
  function start() {
    if (_timer) return;
    sincronizar(); // roda imediatamente
    _timer = setInterval(sincronizar, INTERVAL);
    console.log("[LiveSync] Iniciado — intervalo 30s");
  }

  function stop() {
    if (_timer) { clearInterval(_timer); _timer = null; }
    console.log("[LiveSync] Parado");
  }
