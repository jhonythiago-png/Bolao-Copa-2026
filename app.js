...
async function importarJogos() {
  ...
  const novos = JOGOS_COPA2026.filter(j =>
    !existing.find(x => x.data === j.data && x.time_a === j.timeA && x.time_b === j.timeB)
  ).map((j, idx) => ({
    id: "j_" + idx + "_" + Math.random().toString(36).slice(2,9),
    data: j.data,
    time_a: j.timeA,
    time_b: j.timeB,
    rodada: j.rodada
  ...
  if (!novos.length) return toast("Todos os jogos já estão cadastrados!", true);

  const btn = document.querySelector('[onclick="importarJogos()"]');
  ...
  try {
    ...
    for (let i = 0; i < novos.length; i += 20) {
      ...
      if (error) { console.error("Erro no lote:", error); erros++; }
    }
    await recarregarTabela("jogos");
    renderAdminJogos();
    populateAdminSelects();
    updateHeroStats();
    if (erros > 0) {
      toast(`⚠️ ${novos.length - erros*20} jogos importados (${erros} lote(s) com erro)`, true);
    } else {
      toast(`✅ ${novos.length} jogos importados!`);
    }
  } catch(e) {
    toast("Erro ao importar: " + e.message, true);
    console.error(e);
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = "📥 Importar Copa 2026"; }
  }
}
