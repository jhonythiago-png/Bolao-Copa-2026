...
function updateHeroStats() {
  const totalJogos = Store.jogos().length;
  const totalParts = Store.participantes().length;
  ["heroJogos","bannerJogos"].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.textContent = totalJogos;
  });
  ["heroParticipantes","bannerParticipantes"].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.textContent = totalParts;
  });
}
...
function computeRanking() {
  ...
  return participantes.map(p => {
    ...
    let totalPts = 0, exatos = 0, parciais = 0, erros = 0, vencedores = 0, jogosFeitos = 0;
    const detalhe = [];
...
function getAvatarColor(nome) {
  ...
  let hash = 0;
  for (let c of nome) hash = c.charCodeAt(0) + ((hash << 5) - hash);
...
function renderRanking() {
  ...
  if (!wrap) {
    ...
    const TOP3_COLORS = ["#f5a623","#b0b8c8","#cd7f32"];
    wrap.innerHTML = ranked.length ? ranked.map((p, i) => {
      const pos     = i + 1;
      const medal   = pos === 1 ? "🥇" : pos === 2 ? "🥈" : pos === 3 ? "🥉" : "";
      const cor3 = pos <= 3 ? ["#f5a623","#b0b8c8","#cd7f32"][i] : "var(--accent)";
      const top3cls = pos <= 3 ? `ranking-card-top${pos}` : "";
      return `
        <div class="ranking-card ${top3cls}" onclick="openPlayerModal('${p.id}')">
          <div class="ranking-card-left">
            <div class="ranking-card-pos">${medal || `<span class="ranking-card-num">${pos}º</span>`}</div>
            <div class="ranking-card-info">
              <div class="ranking-card-nome">${p.nome}</div>
          ...
          <div class="ranking-card-pts" style="color:${cor3}">${p.totalPts}<span class="ranking-card-pts-lbl">pts</span></div>
        </div>`;
    }).join("") : '<div class="empty-row">Nenhum participante cadastrado</div>';
  }
  ...
  const proxWrap = document.getElementById("proximosJogos");
  if (proxWrap) proxWrap.innerHTML = proximos.length
    ? proximos.map(j => `
...
function renderJogos() {
  ...
  const wrap = document.getElementById("jogosCards");
  ...
  if (!wrap) {
    ...
    filtered.forEach(j => {
      ...
      const status = res?.ao_vivo
        ? '<span class="badge badge-live">🔴 Ao Vivo</span>'
        : res ? '<span class="badge badge-done">✅ Finalizado</span>'
              : '<span class="badge badge-pending">⏳ Aguardando</span>';
      const score = res ? `${res.gols_a} × ${res.gols_b}` : '—';
      const tr = document.createElement("tr");
      tr.innerHTML = `<td>${fmtDataLong(j.data)}</td><td>${j.time_a}</td>
        <td style="text-align:center">${score}</td><td>${j.time_b}</td><td>${status}</td>`;
      tbody.appendChild(tr);
    });
    return;
  }
  ...
  wrap.innerHTML = Object.entries(porData).map(([data, js]) => `
    <div class="jogo-date-group">
      <div class="jogo-date-header">📅 ${fmtDataLong(data)}</div>
      ${js.map(j => {
        const res = resultados.find(r => r.jogo_id === j.id);
        const aoVivo = res?.ao_vivo;
        const finalizado = res && !aoVivo;
        const statusCls = aoVivo ? "jogo-card-live" : finalizado ? "jogo-card-done" : "jogo-card-pending";
        const statusLabel = aoVivo
          ? '<span class="jogo-status-live">🔴 AO VIVO</span>'
          : finalizado
            ? '<span class="jogo-status-done">✅ FINALIZADO</span>'
            : '<span class="jogo-status-pending">⏳ AGUARDANDO</span>';
        const scoreHtml = res
          ? `<div class="jogo-score">${res.gols_a}<span class="jogo-score-sep">×</span>${res.gols_b}</div>`
          : `<div class="jogo-score-empty">vs</div>`;
        return `
          <div class="jogo-card ${statusCls}">
            <div class="jogo-card-inner">
              <div class="jogo-team jogo-team-a">${j.time_a}</div>
              ${scoreHtml}
              <div class="jogo-team jogo-team-b">${j.time_b}</div>
            </div>
            <div class="jogo-card-footer">${statusLabel}</div>
          </div>`;
      }).join("")}
    </div>`).join("") || '<div class="empty-row">Nenhum jogo cadastrado</div>';
}
...
function renderPremiacao() {
  ...
  const EMOJIS = ["👑","🥈","🥉","",""];
  const CORES  = ["#f5a623","#b0b8c8","#cd7f32","#00d4ff","#7c8da6"];
  ...
  if (grid) {
    grid.innerHTML = PREMIOS_PCT.map((p, i) => {
      ...
      const posLabel = EMOJIS[i]
        ...
        : `<span class="premio-card-pos" style="color:${cor}">${p.pos}</span>`;
      return `
        <div class="${spanCls}" style="--cor:${cor}">
          <div class="premio-card-top">
            ${posLabel}
  ...
  const TOP3C = ["#f5a623","#b0b8c8","#cd7f32"];
  ranked.forEach((p, i) => {
    const slot    = PREMIOS_PCT[i];
    const pos     = i + 1;
    const icon    = pos===1?"🥇":pos===2?"🥈":pos===3?"🥉":`${pos}º`;
    const avColor = pos <= 3 ? TOP3C[i] : getAvatarColor(p.nome);
    const tr      = document.createElement("tr");
...
const LiveSync = (() => {
  ...
  const TEAM_MAP = {
    ...
    "Cuba":"Cuba","Haiti":"Haiti","Curaçao":"Curaçao",
    // América do Sul
    ...
    "England":"Inglaterra","Italy":"Itália","Croatia":"Croácia",
    "Switzerland":"Suíça","Denmark":"Dinamarca","Norway":"Noruega",
    "Sweden":"Suécia","Austria":"Áustria","Scotland":"Escócia",
    "Czech Republic":"República Tcheca","Czechia":"República Tcheca",
    "Bosnia and Herzegovina":"Bósnia","Bosnia-Herzegovina":"Bósnia",
    "Serbia":"Sérvia","Ukraine":"Ucrânia","Romania":"Romênia",
    "Hungary":"Hungria","Poland":"Polônia","Slovakia":"Eslováquia",
    "Slovenia":"Eslovênia","Greece":"Grécia","Albania":"Albânia",
    "Turkey":"Turquia","Türkiye":"Turquia",
    "Georgia":"Geórgia","Iceland":"Islândia","Wales":"País de Gales",
    "North Macedonia":"Macedônia do Norte","Finland":"Finlândia",
    // África
    ...
    "Iraq":"Iraque","Australia":"Austrália","New Zealand":"Nova Zelândia",
    "Uzbekistan":"Uzbequistão","Jordan":"Jordânia","Oman":"Omã",
    "United Arab Emirates":"Emirados Árabes","Bahrain":"Bahrein",
    "China":"China","India":"Índia","Thailand":"Tailândia",
    "Vietnam":"Vietnã","Indonesia":"Indonésia","Malaysia":"Malásia",
    "Philippines":"Filipinas","Kuwait":"Kuwait","Syria":"Síria",
    "Tajikistan":"Tadjiquistão","Kyrgyzstan":"Quirguistão",
  };
...
// ===== SERVICE WORKER — PWA + Sync silencioso =====
  ...
  window.addEventListener('load', async () => {
    try {
      ...
      navigator.serviceWorker.addEventListener('message', event => {
        if (event.data?.type === 'SILENT_SYNC') {
          LiveSync.sincronizar();
        }
      });

      // Avisa SW para iniciar o timer de sync
      if (reg.active) {
        reg.active.postMessage({ type: 'START_SYNC' });
      }
      ...
      console.warn('[PWA] SW não registrado:', e.message);
    }
  });
}
